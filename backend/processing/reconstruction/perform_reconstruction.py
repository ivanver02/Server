...
def load_frame_keypoints(patient_id: str, session_id: str, chunk_id: int, frame_id: int, camera_ids: List[str]) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
    """
    Carga keypoints 2D de un frame específico para una lista de cámaras.
...
def create_cameras_from_config(camera_ids: List[str]) -> Dict[str, Camera]:
    """Crea objetos Camera usando la configuración de intrínsecos para una lista de cámaras"""
    cameras = {}
...
def get_camera_ids(base_path: Path) -> List[str]:
    """
    Obtiene una lista de IDs de cámaras basada en las carpetas disponibles en el directorio base.

    Args:
        base_path: Ruta base donde se encuentran las carpetas de las cámaras.

    Returns:
        Lista de IDs de cámaras detectadas.
    """
    if not base_path.exists():
        logger.warning(f"No se encontró el directorio base: {base_path}")
        return []

    # Detectar carpetas que comiencen con "camera"
    camera_ids = [folder.name for folder in base_path.iterdir() if folder.is_dir() and folder.name.startswith("camera")]
    logger.info(f"Cámaras detectadas: {camera_ids}")
    return camera_ids


def get_all_available_frames(patient_id: str, session_id: str) -> Dict[int, List[int]]:
    """
    Obtiene todos los frames disponibles organizados por chunk para las cámaras detectadas dinámicamente.
    Devuelve un diccionario {chunk_id: [lista_de_frame_ids]}.
    """
    base_path = _ROOT / "data" / "processed" / "2D_keypoints" / f"patient{patient_id}" / f"session{session_id}"

    camera_ids = get_camera_ids(base_path)
    if not camera_ids:
        logger.warning("No se detectaron cámaras disponibles.")
        return {}
...
def calculate_session_extrinsics(patient_id: str, session_id: str, available_frames: Dict[int, List[int]], max_chunk: int, camera_ids: List[str]) -> Optional[Dict[str, Camera]]:
    """
    ...
    # Recopilar matrices extrínicas de múltiples chunks
    collected_extrinsics = {cam_id: {'R_matrices': [], 't_vectors': []} for cam_id in camera_ids}
    ...
    for chunk_id in range(max_chunk + 1):
        ...
        try:
            ...
            frame_keypoints = load_frame_keypoints(patient_id, session_id, chunk_id, target_frame_id, camera_ids)
            ...
            cameras = create_cameras_from_config(camera_ids)
    ...
    try:
        averaged_cameras = create_cameras_from_config(camera_ids)
...
def process_chunk_parallel(chunk_data: Tuple[int, List[int], str, str, Dict[str, Camera], float]) -> Dict[str, any]:
    ...
    for frame_id in frame_list:
        try:
            ...
            # Replace hardcoded camera references with dynamic detection
            base_path = _ROOT / "data" / "processed" / "2D_keypoints" / f"patient{patient_id}" / f"session{session_id}"
            camera_ids = get_camera_ids(base_path)

            # Update the function call to use dynamically detected cameras
            frame_keypoints = load_frame_keypoints(patient_id, session_id, chunk_id, frame_id, camera_ids)
...
def start_3d_reconstruction(patient_id: str, session_id: str, max_chunk: int, person_height_cm: float = 190.0, use_parallel: bool = True):
    ...
    session_cameras = calculate_session_extrinsics(patient_id, session_id, available_frames, max_chunk, camera_ids)
